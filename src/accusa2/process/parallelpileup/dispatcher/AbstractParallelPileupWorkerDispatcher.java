...
public abstract class AbstractParallelPileupWorkerDispatcher<T extends AbstractParallelPileupWorker> {
		...
		synchronized (this) {
			...
			while(hasNext() || !threadContainer.isEmpty()) {
				...
				while(threadContainer.size() < parameters.getMaxThreads() && hasNext()) {
					T processParallelPileupThread = buildParallelPileupWorker();
	...
	abstract protected T buildParallelPileupWorker();
	...
	public SAMFileReader createBAMFileReader1() {
		return initReader(parameters.getPathname1());
	}

	public SAMFileReader createBAMFileReader2() {
		return initReader(parameters.getPathname2());
	}
